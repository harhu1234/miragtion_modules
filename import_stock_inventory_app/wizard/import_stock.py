...
class Stock_Import(models.TransientModel):
    ...
    def import_file_button(self):
        ...
        if self.inventory_option == 'add':
            stock_inventory = inventory_obj.create({'name': self.name, 'location_ids': self.location_ids.ids})
            stock_inventory.action_start()

        elif self.inventory_option == 'update':
            stock_inventory = inventory_obj.search([('name', '=', self.name), ('location_ids', '=', self.location_ids.ids)], limit=1)
                ...
                if no <= 0:
                    ...
                    if data:
                        ...
                        if product_rec.tracking != 'none':
                            ...
                            lot_num_rec = self.env['stock.production.lot'].search([('name', '=', values['lot']),('company_id', '=', company_id.id)], limit=1)

                            if not lot_num_rec:

                                lot_num_rec = self.env['stock.production.lot'].create({'name': values['lot'], 'product_id': product_rec.id, 'company_id': company_id.id})
