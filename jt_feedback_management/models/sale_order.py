...
class SaleOrder(models.Model):
    ...
    survey_input_ids = fields.One2many('survey.user_input', 'sale_id', string='Answers')
    ...
    def send_feedback_link(self):
        ...
        if sale_template_id:
            template = self.env['mail.template'].sudo().browse(sale_template_id)

        # Selecting default survey form
        survey = self.env.ref('jt_feedback_management.survey_survey_1',
                              raise_if_not_found=False)

        config = self.env['ir.config_parameter'].sudo()
        sale_survey_id = int(config.get_param('jt_feedback_management.sale_survey_id'))
        if sale_survey_id:
            survey = self.env['survey.survey'].sudo().browse(sale_survey_id)

        local_context = dict(
            self.env.context,
            default_survey_id=survey and survey.id or False,
            default_model_id=self.env.ref('sale.model_sale_order').id,
