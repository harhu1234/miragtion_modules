...
class ExcelPackage(models.TransientModel):

    _name = 'export.excel.package'
    _description = 'Excel Package'

    excel_file = fields.Binary(string='File')
    filename = fields.Char(string='File Name')


    def print_package_excel(self):

        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output,{'in_memory':True})
        sheet = workbook.add_worksheet('Package')
        ...
        package_ids = self.env['pacakge.package'].browse(self.env.context.get('active_ids'))

        row += 9
        col = 0

        for package in package_ids:
            sheet.write(row, col, ('Package'), header_style)
            col += 1
            sheet.write(row, col, ('Type'), header_style)
            col += 1
            sheet.set_column(col, col, 15)
            sheet.write(row, col, ('Regular Price'), header_style)
            col += 1
            sheet.set_column(col, col, 15)
            sheet.write(row, col, ('Discount'), header_style)
            col += 1
            sheet.set_column(col, col, 15)
            sheet.write(row, col, ('Package Price'), header_style)

            row += 1
            col = 0

            sheet.write(row,col,package.name,data_style)
            col += 1
            sheet.write(row,col,package.package_type,data_style)
            col += 1
            sheet.write(row,col,package.regular_price)
            col += 1
            sheet.write(row,col,package.discount)
            col += 1
            sheet.write(row,col,package.package_price,price_style)
            ...
            row += 1
            col = 0
            for service in package.line_ids:
                sheet.write(row,col,service.service_id.name,data_style)
                col += 1
                sheet.write(row,col,service.description,data_style)
                col += 1
        ...
        res_id = self.env['export.excel.package'].create({'excel_file':xls_data,'filename':filename_1})

        return {
            'name' : 'Package File',
            'view_mode' : 'form',
            'res_model' : 'export.excel.package',
            'view_id' : False,
            'domain' : [],
