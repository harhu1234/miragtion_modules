...
class SupportTeam(models.Model):
    ...
    member_ids = fields.One2many('res.users', 'klachten_team_id', string='Team Members')
    ...
    def _get_default_team_id(self, user_id=None):
        ...
        if 'default_team_id' in self.env.context:
            team_id = self.env['klachten.team'].browse(self.env.context.get('default_team_id'))
        if not team_id or not team_id.exists():
            team_id = self.env['klachten.team'].sudo().search(
                ['|', ('user_id', '=', user_id), ('member_ids', '=', user_id)],
                limit=1)
        if not team_id:
            team_id = self.env.ref('klachten.team_alpha', raise_if_not_found=False)
        return team_id


    def get_alias_model_name(self, vals):
        return 'klachten.ticket'
