...
class AccountMove(models.Model):
    ...
    def send_feedback_link(self):
        ...
        if invoice_template_id:
            template = self.env['mail.template'].sudo().browse(invoice_template_id)

        # Selecting default survey form
        survey = self.env.ref('jt_feedback_management.survey_survey_3',
                              raise_if_not_found=False)

        config = self.env['ir.config_parameter'].sudo()
        invoice_survey_id = int(config.get_param(
            'jt_feedback_management.invoice_survey_id'))
        if invoice_survey_id:
            survey = self.env['survey.survey'].sudo().browse(invoice_survey_id)

        local_context = dict(
            self.env.context,
            default_survey_id=survey and survey.id or False,
            default_model_id=self.env.ref('account.model_account_move').id,
