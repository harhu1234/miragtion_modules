...
class StockPicking(models.Model):
    ...
    def send_feedback_link(self):
        ...
        if stock_template_id:
            template = self.env['mail.template'].sudo().browse(stock_template_id)

        # Selecting default survey form
        survey = self.env.ref('jt_feedback_management.survey_survey_4',
                              raise_if_not_found=False)

        config = self.env['ir.config_parameter'].sudo()
        stock_survey_id = int(config.get_param('jt_feedback_management.stock_survey_id'))
        if stock_survey_id:
            survey = self.env['survey.survey'].sudo().browse(stock_survey_id)

        local_context = dict(
            self.env.context,
            default_survey_id=survey and survey.id or False,
            default_model_id=self.env.ref('stock.model_stock_picking').id,
