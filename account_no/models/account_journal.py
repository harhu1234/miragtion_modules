...
class AccountJournal(models.Model):
    ...
    def _default_outbound_payment_methods(self):
        ...
        return vals + self.env.ref('account_no.account_payment_method_sepa_ct')
    ...
    def _enable_sepa_ct_on_bank_journals(self):
        ...
        """
        sepa_ct = self.env.ref('account_no.account_payment_method_sepa_ct')
        nok = self.env.ref('base.NOK')
        if self.env.company.currency_id == nok:
            domain = ['&', ('type', '=', 'bank'), '|', ('currency_id', '=', nok.id), ('currency_id', '=', False)]
        else:
            domain = ['&', ('type', '=', 'bank'), ('currency_id', '=', nok.id)]
    ...
    def create_iso20022_credit_transfer(self, payments, batch_booking=False, sct_generic=False):
        ...
        """
        pain_version = self.company_id.norway_pain_version
    ...
    def _get_company_PartyIdentification32(self, sct_generic=False, org_id=True, postal_address=True, nm=True, issr=True, schme_nm=False):
        ...
        if nm:
            Nm = etree.Element("Nm")
            Nm.text = sanitize_communication(company.norway_initiating_party_name[:name_length])
            ret.append(Nm)
        ...
        if org_id:
            if not company.norway_orgid_id:
                raise UserError(_("Please first set a Norway identification number in the accounting settings."))
            Id = etree.Element("Id")
            OrgId = etree.SubElement(Id, "OrgId")
            Othr = etree.SubElement(OrgId, "Othr")
            _Id = etree.SubElement(Othr, "Id")
            _Id.text = sanitize_communication(company.norway_orgid_id)
            if issr and company.norway_orgid_issr:
                Issr = etree.SubElement(Othr, "Issr")
                Issr.text = sanitize_communication(company.norway_orgid_issr)
            if schme_nm:
    ...
    def _get_CdtTrfTxInf(self, PmtInfId, payment, sct_generic, pain_version, local_instrument=None):
        ...
        max_digits = val_Ccy == 'NOK' and 11 or 15
    ...
    def _get_CdtrAcct(self, bank_account, sct_generic):
        if not sct_generic and (not bank_account.acc_type or not bank_account.acc_type == 'iban'):
            raise UserError(_("The account %s, linked to partner '%s', is not of type IBAN.\nA valid IBAN account is required to use Norway features.") % (bank_account.acc_number, bank_account.partner_id.name))
