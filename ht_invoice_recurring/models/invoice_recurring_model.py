...
class InvoiceRecurring(models.Model):
    ...
    ht_invoice_recurring_count = fields.Integer(
        string='# of Invoices', compute='_compute_ht_invoice_recurring_order_compute')
    ...
    def _compute_ht_invoice_recurring_order_compute(self):
        invoice_obj = self.env['account.move']
        if self:
            for rec in self:
                rec.ht_invoice_recurring_count = 0
                count = invoice_obj.sudo().search_count([
                    ('ht_invoice_recurring_order_id', '=', rec.id),
                    ('move_type', 'in', ['out_invoice'])
                ])
                rec.ht_invoice_recurring_count = count

    def action_view_recurring_order(self):
        if self:
            invoice_obj = self.env['account.move']
            invoices = invoice_obj.sudo().search([
                ('ht_invoice_recurring_order_id', '=', self.id),
                ('move_type', 'in', ['out_invoice'])
            ])
