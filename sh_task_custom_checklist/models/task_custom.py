...
class ProjectTask(models.Model):
	...
	def _compute_custom_checklist(self):
		...
		total_cnt = self.env['task.custom.checklist.line'].search_count([('task_id','=',self.id),('state','!=','cancelled')]) 
		compl_cnt = self.env['task.custom.checklist.line'].search_count([('task_id','=',self.id),('state','=','completed')]) 
