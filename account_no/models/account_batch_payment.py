...
def check_valid_SEPA_str(string):
    if re.search('[^-A-Za-z0-9/?:().,\'+ ]', string) is not None:
        raise ValidationError(_("The text used in Norway files can only contain the following characters :\n\n"
...
class AccountBatchPayment(models.Model):
    ...
    sct_batch_booking = fields.Boolean(string="NCT Batch Booking", default=True, help="Request batch booking from the bank for the related bank statements.")
    sct_generic = fields.Boolean(compute='_compute_sct_generic',
        help=u"Technical feature used during the file creation. A Norway message is said to be 'generic' if it cannot be considered as "
    ...
    def validate_batch(self):
        ...
        if self.payment_method_code == 'sepa_ct':
            if self.journal_id.bank_account_id.acc_type != 'iban':
                raise UserError(_("The account %s, of journal '%s', is not of type IBAN.\nA valid IBAN account is required to use Norway features.") % (self.journal_id.bank_account_id.acc_number, self.journal_id.name))
    ...
    def _get_sct_genericity_warnings(self):
        ...
        for payment in self.mapped('payment_ids'):
            ...
            if payment.currency_id.name != 'NOK' and (self.journal_id.currency_id or self.journal_id.company_id.currency_id).name == 'NOK':
    ...
    def check_payments_for_errors(self):
        ...
        for payment in self.payment_ids.filtered(lambda x: x.state == 'posted'):
            ...
            max_digits = payment.currency_id.name == 'NOK' and 11 or 15
    ...
    def _generate_export_file(self):
        if self.payment_method_code == 'sepa_ct':
            ...
            return {
                ...
                'filename': "NCT-" + self.journal_id.code + "-" + str(fields.Date.today()) + ".xml",
            }
    ...
    def _generate_payment_template(self, payments):
        ...
        for payment in payments:
            ...
            payment_dict = {
                'id' : payment.id,
                'name': payment.ref or 'NCT-' + self.journal_id.code + '-' + str(fields.Date.today()),
